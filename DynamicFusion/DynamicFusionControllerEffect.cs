...
namespace YMM4_shape_fusion_plugin.DynamicFusion
{
	...
	internal class DynamicFusionControllerEffectProcessor : IVideoEffectProcessor, IDisposable
	{
		...
		public DrawDescription Update(EffectDescription desc)
		{
			...
			// 合体形状は既に絶対キャンバス座標で構築するため、制御アイテム自身の位置・回転・拡大率・カメラは
			// 打ち消しておかないと二重に変形がかかってしまう(LineControllerEffectの手法と同じ考え方)。
			// TODO: v1はカメラを考慮しないためCamera=Identityに固定。カメラ演出付きのシーンでは別途対応が必要。
			var d = desc.DrawDescription;
			return d with
			{
				Draw = new Vector3(0f, 0f, d.Draw.Z),
